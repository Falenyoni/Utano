...
public class GetBrandingHandler(IdentityDbContext db, IFileAttachmentLookup fileLookup)
    : IRequestHandler<GetBrandingQuery, BrandingDto>
{
    public async Task<BrandingDto> Handle(GetBrandingQuery request, CancellationToken ct)
    {
        ...
        return new BrandingDto(practice.Name, practice.PrimaryColor, logoUrl);
    }
}
